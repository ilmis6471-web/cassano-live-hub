...
const DISCORD_ROLE_IDS=(process.env.DISCORD_ROLE_IDS||'').split(',').map(x=>x.trim()).filter(Boolean);
...
const server=http.createServer(async(req,res)=>{
 try{
  ...
  if(req.url==='/api/admin/discord-test'&&req.method==='POST'){if(!auth(req))return json(res,401,{error:'Unauthorized'});if(!DISCORD_WEBHOOK_URL)return json(res,400,{error:'DISCORD_WEBHOOK_URL belum diatur'});const content=(DISCORD_ROLE_IDS.length?DISCORD_ROLE_IDS.map(id=>'<@&'+id+'>').join(' ')+'\\n':'')+'🟢 **Cassano Live Hub — Test Notification**';const r=await fetch(DISCORD_WEBHOOK_URL,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({content,embeds:[{title:'Discord terhubung!',description:'Notifikasi Cassano Live Hub siap digunakan.',color:5763719,footer:{text:'Cassano Live Hub'}}],allowed_mentions:{parse:[],roles:DISCORD_ROLE_IDS}})});if(!r.ok)return json(res,502,{error:'Discord menolak webhook'});return json(res,200,{ok:true,roles_notified:DISCORD_ROLE_IDS.length})}
